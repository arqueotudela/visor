(function (factory, window) {
    ...
    L.MeasureAction = L.Handler.extend({
        ...
        // Nuevo método para borrar la medición actual TOÑO
        _clearNotFinishedMeasurement: function () {
            if (this._measurementStarted && this._trail && this._trail.overlays) {
                this._map.removeLayer(this._trail.overlays);
                this._trail.overlays = L.featureGroup();
                this._map.addLayer(this._trail.overlays);
        ...
        _finishMeasure: function (event) {
            ...
            if (this._map && this._map.activeTool) {
                this._map.activeTool(null);
            }
            ...
            if (!this._directPath) {
                if (this.options.model === "area") {
                    this._directPath = new L.Polygon([latlng], {
                        weight: 4, // Cambio TOÑO this.options.weight
                        color: this.options.color,
                        dashArray: [5, 5],
                        fillOpacity: 0,
                        interactive: false,
        ...
        _addMarker: function (latLng) {
            var marker = new L.CircleMarker(latLng, {
                color: this.options.color,
                opacity: 1,
                weight: 2, //this.options.weight, //TOÑO ojo que esto era la mitad de lo que está en otros, era 1 y en otros 2
                fillColor: "#FFFFFF",
                fill: true,
                fillOpacity: 1,
                radius: 3,
